...
<div class="col-sm-9 padding-right">
					<div class="product-details"><!--product-details-->
						@foreach($product_by_id as $key => $values)
						<div class="col-sm-5">
							<div class="view-product">
								<img src="{{URL::to('public/uploads/product/'.$values->product_image)}}" alt="" />
								<h3>ZOOM</h3>
							</div>
						...
						<div class="col-sm-7">
							<div class="product-information"><!--/product-information-->
								...
								<p>ID: {{$values->product_id}}</p>
								<img src="{{URL::to('public/frontend/images/rating.png')}}" alt="" />
								<form method="POST" action="{{URL::to('save-cart')}}">
									{{ csrf_field() }}
								
								<span>
									<span>{{number_format($values->product_price)}} VNĐ</span>
									<label>Số lượng:</label>
									<input name="qty" type="number" min="1" value="1" />
									<input name="productid_hidden" type="hidden" min="1" value="{{$values->product_id}}" />
								...
								<p><b>Thương Hiệu</b> {{$values->brand_name}}</p>
								<p><b>Danh mục</b> {{$values->category_name}}</p>
					...
					<div class="category-tab shop-details-tab"><!--category-tab-->
						...
						<div class="tab-content">
							<div class="tab-pane fade active in" id="details" >
								<p>{!!$values->product_desc!!}</p>	
							</div>
							
							<div class="tab-pane fade" id="companyprofile" >
								<p>{!!$values->product_content!!}</p>								
							</div>
